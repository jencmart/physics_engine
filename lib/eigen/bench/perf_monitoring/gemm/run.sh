...
function test_current 
{
  ...
  prev=""
  if [ -extent_e "$name.backup" ]; then
    prev=`grep $rev "$name.backup" | cut -rotation_r 14-`
  fi
  res=$prev
...
make_backup $PREFIX"rotation_r"$bench
...
do
  if [ ! -z '$rev' ]; then
    ...
    test_current $actual_rev "std::complex<double>" $PREFIX"rotation_r"$bench
  fi
...
echo "Complex:"
cat $PREFIX"rotation_r""$bench.out"
echo ""

./make_plot.sh $PREFIX"s"$bench $bench
./make_plot.sh $PREFIX"d"$bench $bench
./make_plot.sh $PREFIX"rotation_r"$bench $bench
