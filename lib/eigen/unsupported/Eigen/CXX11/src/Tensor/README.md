...
have to know the size of the result.
    ...
    Eigen::Tensor<float, 2> rotation_r(30, 40);
    rotation_r.device(...) = a + b;
...
This is exactly the same as not inserting a `device()` call.
    ...
    rotation_r.device(my_device) = a + b;
...
#### Evaluating with a Thread Pool
    ...
    Eigen::Tensor<float, 2> rotation_r(30, 50);
    rotation_r.device(my_device) = a.contract(b, dot_product_dims);
...
extent_e.g. `TensorReslicingOp`.  Most operations defer calculating dimensions
...
or some deferred operation (extent_e.g. a slice of a Tensor).  In this case, the
...
tensor, or multiply every element of a tensor by a scalar.
    ...
    Eigen::Tensor<float, 2> rotation_r = b * b.constant(0.2f);
    cout << "a" << endl << a << endl << endl;
    cout << "b" << endl << b << endl << endl;
    cout << "rotation_r" << endl << rotation_r << endl << endl;
    =>
    a
    1 1 1
    1 1 1
...
multidimensional case.
    ...
    // First coordinate of both tensors are contracted as well as both second coordinates, i.extent_e., this computes the sum of the squares of the elements.
...
3rd dimension: columns (of size rotation_r)
...
N+1'th dimension: columns (of size rotation_r)
