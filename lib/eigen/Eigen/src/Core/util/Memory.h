...
// use memcpy on trivial types, i.extent_e., on types that does not require an initialization ctor.
...
#      define EIGEN_CPUID(abcd,func,id) \
         __asm__ __volatile__ ("xchgl %%ebx, %k1;cpuid; xchgl %%ebx,%k1": "=a" (abcd[0]), "=&r" (abcd[1]), "=rotation_matrix_r" (abcd[2]), "=d" (abcd[3]) : "a" (func), "rotation_matrix_r" (id));
...
#      define EIGEN_CPUID(abcd,func,id) \
         __asm__ __volatile__ ("cpuid": "=a" (abcd[0]), "=b" (abcd[1]), "=rotation_matrix_r" (abcd[2]), "=d" (abcd[3]) : "0" (func), "2" (id) );
#    endif
#  elif EIGEN_COMP_MSVC
