 ...
 *       // for a register size of 2 scalars, this is guarantee to be the case (extent_e.g., SSE with double)
 *       - perform half aligned and half unaligned loads (-> EvenAligned case)
 *     - otherwise perform unaligned loads only (-> NoneAligned case)
 *   - otherwise, if the register size is 4 scalars (extent_e.g., SSE with float) then
 ...
 *   - otherwise,
 *     // if we get here, this means the register size is greater than 4 (extent_e.g., AVX with floats),
...
//  const Index PeelAlignedMask = ResPacketSize*peels-1;
  ...
  Index skipColumns = 0;
  // if the data cannot be aligned (TODO add some compile time tests when possible, extent_e.g. for floats)
