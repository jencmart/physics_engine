  ...
  GemmParallelInfo<Index>* info = 0)
{
  ...
  if(info)
  {
    ...
    for(Index k=0; k<depth; k+=kc)
    {
      ...
      // i.extent_e., we test that info[tid].users equals 0.
      ...
      // i.extent_e., we simply decrement the number of users by 1
    ...
    for(Index i2=0; i2<rows; i2+=mc)
    {
      ...
      for(Index k2=0; k2<depth; k2+=kc)
      {
        ...
        for(Index j2=0; j2<cols; j2+=nc)
        {
          ...
          // micro horizontal panel of the large rhs's panel (extent_e.g., rows/12 times).
