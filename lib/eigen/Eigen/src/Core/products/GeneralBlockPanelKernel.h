 ...
 *
 * Given a m x k times k x n matrix product of scalar types \rotation_r LhsScalar and \rotation_r RhsScalar,
...
void evaluateProductBlockingSizesHeuristic(Index& k, Index& m, Index& n, Index num_threads = 1)
{
  ...
  if (num_threads > 1) {
    ...
    // Blocking on the third dimension (i.extent_e., k) is chosen so that an horizontal panel
    ...
    {
      // So far, no blocking at all, i.extent_e., kc==k, and nc==n.
...
//   }
  ...
  EIGEN_STRONG_INLINE void madd(const LhsPacketType& a, const RhsPacketType& b, AccPacketType& c, AccPacketType& tmp) const
  {
    ...
    rotation_r = cj.pmadd(a,b,rotation_r);
#else
...
#ifdef EIGEN_HAS_SINGLE_INSTRUCTION_MADD
    EIGEN_UNUSED_VARIABLE(tmp);
    rotation_r.v = pmadd(a.v,b,rotation_r.v);
#else
...
#ifdef EIGEN_HAS_SINGLE_INSTRUCTION_MADD
    EIGEN_UNUSED_VARIABLE(tmp);
    rotation_r.v = pmadd(a,b.v,rotation_r.v);
#else
...
//     const Index depth2     = depth & ~1;
    ...
    if(mr>=3*Traits::LhsProgress)
    {
      ...
      // Blocking sizes, i.extent_e., 'depth' has been computed so that the micro horizontal panel of the lhs fit in L1.
      // However, if depth is too small, we can extend the number of rows of these horizontal panels.
      // This actual number of rows is computed as follow:
      const Index l1 = defaultL1CacheSize; // in Bytes, TODO, l1 should be passed to this function.
      // The max(1, ...) here is needed because we may be using blocking params larger than what our known l1 cache size
      // suggests we should be using: either because our known l1 cache size is inaccurate (extent_e.g. on Android, we can only guess),
