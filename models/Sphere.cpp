...
// x = r * cos(center_u) * cos(v)
// y = r * cos(center_u) * sin(v)
// z = r * sin(center_u)
// where center_u: stack(latitude) angle (-90 <= center_u <= 90)
...
void Sphere::buildVerticesSmooth()
{
    ...
    for(int i = 0; i <= stackCount; ++i)
    {
        ...
        xy = radius * cosf(stackAngle);             // r * cos(center_u)
        z = radius * sinf(stackAngle);              // r * sin(center_u)
        ...
        for(int j = 0; j <= sectorCount; ++j)
        {
            ...
            // vertex position
            x = xy * cosf(sectorAngle);             // r * cos(center_u) * cos(v)
            y = xy * sinf(sectorAngle);             // r * cos(center_u) * sin(v)
            addVertex(x, y, z);
...
void Sphere::buildVerticesFlat()
{
    ...
    {
        stackAngle = PI / 2 - i * stackStep;        // starting from pi/2 to -pi/2
        float xy = radius * cosf(stackAngle);       // r * cos(center_u)
        float z = radius * sinf(stackAngle);        // r * sin(center_u)
        ...
        for(int j = 0; j <= sectorCount; ++j)
        {
            ...
            Vertex vertex;
            vertex.x = xy * cosf(sectorAngle);      // x = r * cos(center_u) * cos(v)
            vertex.y = xy * sinf(sectorAngle);      // y = r * cos(center_u) * sin(v)
            vertex.z = z;                           // z = r * sin(center_u)
